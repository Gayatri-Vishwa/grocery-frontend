...
const Auth = () => {
    ...
    const {setShowUserLogin,setUser,fetchUser,axios,setCartItems ,navigate}=useContext(AppContext)
    ...
    const { data } = await axios.post(
      ...
      { withCredentials: true }
        ...
        if(data.success){
            ...
            setUser(data.user)
             fetchUser()
            setShowUserLogin(false)      //auth show

           
  setCartItems({ ...data.user.cartItems }); // clone
            ...
            {state === "register" ? (
                <p>
                    Already have account? <span onClick={() => {setState("login")
                         setName(""); // 🔥 important
                         }} className="text-indigo-500 cursor-pointer">click here</span>
                </p>
            ) : (
                <p>
